...
- Bug in :meth:`RangeIndex.symmetric_difference` returning an unsorted result when the other index was empty and the index had a negative step (e.g. ``RangeIndex(-2, -5, -2)``), instead of the sorted result that non-empty symmetric differences produce (:issue:`68264`)
